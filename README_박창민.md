![01 24779032 1](https://user-images.githubusercontent.com/82860336/119298833-c3be1f80-bc98-11eb-9c69-37e7ed7ed60a.jpg)
...
기능적 요구사항
- 호스트(방주인)이 임대할 방을 등록한다.
- 호스트(방주인)이 등록한 방의 내용을 수정/삭제 할 수 있다.
- 고객이 방을 예약한다.
- 예약과 동시에 결제가 이루어 진다.
- 고객이 결제 완료 시 예약이 확정된다.
...
2. 장애격리
    - Room 관리 기능이 수행되지 않더라도 예약은 365일 24시간 받을 수 있어야 한다  Async (event-driven), Eventual Consistency
    - Payment 시스템이 과중되면 사용자를 잠시동안 받지 않고 결제를 잠시후에 하도록 유도한다 Circuit breaker, fallback
3. 성능
    - 고객이 자주 등록된 모든 방의 상태 및 예약 가능 여부 등을  종합적으로 확인할 수 있는 프론트엔드 별도로 제공된다. (CQRS)
    - 예약확정시 카톡 등으로 알림을 줄 수 있어야 한다  Event driven
